...
class LocalModel(object):
	...
	def update_weights(self, model, global_round):
		...
		q_w = quantize_weights(new_w, self.device)
...
def quantize_weights(w, device):
	for key, value in w.items():
		...
		if len(val_arr) != 1:
			#[source paper: FEDERATED LEARNING : STRATEGIES FOR IMPROVING COMMUNICATION EFFICIENCY]
			for i, val in enumerate(val_arr):
				p_max = (val_arr[i]-min_val)/(max_val-min_val) 
				p_min = (max_val-val_arr[i])/(max_val-min_val)
				if p_max > p_min:
					val_arr[i] = max_val
				else:
					val_arr[i] = min_val
